  ...
  return (
    <div className={`ed-fp ${expired ? 'is-over' : ''}`}>
      ...
      <div className="ed-fp-right">
        ...
        <button type="button" className="ed-btn ed-fp-ghost" onClick={onPreview}><Eye className="w-4 h-4" /> Student view</button>
        <button type="button" className="ed-fp-end" aria-label="End session" onClick={onEnd}><X className="w-4 h-4" /></button>
      </div>
      <style>{`
        .ed-fp { display: flex; align-items: center; justify-content: space-between; gap: 16px; flex-wrap: wrap; background: #131417; border: none; border-radius: 20px; padding: 20px 24px; margin-bottom: 32px; animation: ed-rise 440ms cubic-bezier(0.2,0.7,0.2,1) both; }
        .ed-fp.is-over { background: #fff; border: 1px solid var(--lq-line); }
        .ed-fp.is-over .ed-fp-title { color: var(--lq-ink); }
        .ed-fp.is-over .ed-fp-sub { color: var(--lq-ink-3); }
        .ed-fp.is-over .ed-fp-clock { color: var(--lq-ink-3); }
        ...
        .ed-fp-title { font-family: var(--font-display); font-weight: 500; font-size: 17px; color: #fff; }
        .ed-fp-sub { font-size: 12.5px; color: rgba(255,255,255,0.62); margin-top: 2px; }
        .ed-fp-right { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
        .ed-fp-code { display: inline-flex; align-items: center; gap: 7px; font-family: var(--font-mono); font-weight: 700; font-size: 15px; letter-spacing: 0.06em; color: #131417; background: #fff; border: none; border-radius: 999px; padding: 8px 15px; cursor: pointer; }
        .ed-fp-code:hover { opacity: 0.88; }
        .ed-fp-clock { font-family: var(--font-mono); font-weight: 700; font-size: 20px; color: var(--ed-accent); min-width: 64px; text-align: center; font-variant-numeric: tabular-nums; }
        .ed-fp-clock.is-over { color: var(--lq-ink-3); }
        .ed-fp-ghost { border: 1px solid rgba(255,255,255,0.25); color: #fff; background: transparent; }
        .ed-fp-ghost:hover { border-color: #fff; }
        .ed-fp-end { display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 999px; border: 1px solid rgba(255,255,255,0.25); background: transparent; color: rgba(255,255,255,0.7); cursor: pointer; }
